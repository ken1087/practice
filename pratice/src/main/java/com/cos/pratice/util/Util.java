...
public class Util {
	...
	public int check(HashMap<String, String[]> search2, String param) {    //チェックするsearch2.get(Index)とパラメータ
		...
		if(param != null) {    //入ったパラメータのデータがある場合
			...
			if(primary[2].equals(param)) {    //primary配列のIndex2（stdnum）とパラメータのstdnumが一致する場合
				return 3;    //returnは3にする
			}
			if(primary[3].equals(param)) {    //primary配列のIndex3（mynum）とパラメータのmynumが一致する場合
				return 4;    //returnは4にする
			}
			if(primary[4].equals(param)) {    //primary配列のIndex4（phone）とパラメータのphoneが一致する場合
