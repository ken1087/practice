...
public class UserDAO{
	public void sava(Users user, UsersPk userpk){ //user : プライマリーキーを除いた値　userpk :　プライマリーきー 
		...
		try {
		    ...
		    u.setUpdateDate(user.getUpdateDate());    //要請した値を生成したUsersオブジェクトにSETする
		    em.persist(u);    	//Save
	...
	public List<HashMap<String, String[]>> findAllList(){
	    ...
	    String jpql = qry.getSelectall();    //MyQueryにSelect u FROM Users uが書いてる変数を呼び出してStringに格納　戻り値：db.propertiesにあるSQL文
		...
		try {
		    ...
		    hashlist = util.hash(list);    //HashMapに格納するためのメソッド 戻り値：HashMapに入れたデータのリスト , パラメータの意味：DBから呼び出したList
