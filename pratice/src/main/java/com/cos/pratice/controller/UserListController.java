...
public class UserListController {
	...
	public String search(Model model, String num,String userid,String stdnum,String mynum,String phone){    //5個のPrimary Keyのデータを要請
		...
		for(int i = 0; i<user.size(); i++) {	
		    ...
		    if(primary[0].equals(num) && num != null) {	    //Num配列と要請したNumの値が同じの場合、要請したNumがある場合
			    search.add(user.get(i)); continue;   //あってる場合、宣言したsearch変数に格納する 、入れると次のループを回す
			}															
	        if(primary[1].equals(userid) && userid != null) {    //Userid配列と要請したUseridの値が同じの場合、要請したUseridがある場合
			    search.add(user.get(i));  continue;  //あってる場合、宣言したsearch変数に格納する、入れると次のループを回す
			}
		    if(primary[2].equals(stdnum) && stdnum != null) {    //Stdnum配列と要請したStdnumの値が同じの場合、要請したStdnumがある場合
			    search.add(user.get(i)); continue;    //あってる場合、宣言したsearch変数に格納する、入れると次のループを回す
			}
		    if(primary[3].equals(mynum) && mynum != null) {    //Mynum配列と要請したMynumの値が同じの場合、要請したMynumがある場合
			    search.add(user.get(i)); continue;    //あってる場合、宣言したsearch変数に格納する、入れると次のループを回す
			}
		    if(primary[4].equals(phone) && phone != null) {    //Phone配列と要請したPhoneの値が同じの場合、要請したPhoneがある場合			
			    search.add(user.get(i)); continue;   //あってる場合、宣言したsearch変数に格納する、入れると次のループを回す
			}
		}
		...
		if(num != null) {
		    for(int k = search.size()-1; k>=0; k--) {
			    ...
			    ck = util.check(search.get(k), num, 1);    //　戻り値は1 or 0    
			    if(ck != 1) {    //ckが1の場合
				    search.remove(k);    //Search2のKのIndexは削除する		
				}																	
			}
		}
		if(userid != null) {
		    for(int k = search.size()-1; k>=0; k--) {
				...
				//userid : パラメータの値
				//intはnum、userid、stdnum、mynum、phone中判断する整数　num:1 userid:2 stdnum:3 mynum:4 phone:5
			    ck = util.check(search.get(k), userid, 2);   //　戻り値は2 or 0 
			    if(ck != 2) {    //ckが2の場合
				    search.remove(k);    //Search2のKのIndexは削除する
				}																	
			}
		...
		if(stdnum != null) {
		    for(int k = search.size()-1; k>=0; k--) {
				...
				//stdnum : パラメータの値
				//intはnum、userid、stdnum、mynum、phone中判断する整数　num:1 userid:2 stdnum:3 mynum:4 phone:5
			    ck = util.check(search.get(k), stdnum, 3);    //　戻り値は3 or 0
			    if(ck != 3) {    //ckが3の場合
				    search.remove(k);    //Search2のKのIndexは削除する
				}
			}
		...
		if(mynum != null) {
		    for(int k = search.size()-1; k>=0; k--) {
			    ...
			    ck = util.check(search.get(k), mynum, 4);    //　戻り値は4 or 0
			    if(ck != 4) {    //ckが4の場合
				    search.remove(k);    //Search2のKのIndexは削除する
				}
			}
		...
		if(phone != null) {
		    for(int k = search.size()-1; k>=0; k--) {
				...
				//phone : パラメータの値
				//intはnum、userid、stdnum、mynum、phone中判断する整数　num:1 userid:2 stdnum:3 mynum:4 phone:5
			    ck = util.check(search.get(k), phone, 5);    //　戻り値は5 or 0
			    if(ck != 5) {    //ckが5の場合
				    search.remove(k);    //Search2のKのIndexは削除する
				}
			}
	    ...
	    model.addAttribute("search", search);    //全ての検査が終わったらModelに格納してJsp fileに渡す
	    return "search";
	}
